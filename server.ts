...
import { normalizeScreeningHistory } from "./src/lib/screeningHistory";
import {
  authMiddleware,
  requireRoles,
  sanitizeUser,
...
// Plain-text parollarni hash qilish (migratsiya)
(function migratePasswordsOnBoot() {
  const users = getUsers();
  if (migratePlainPasswords(users)) {
    saveUsers(users);
...
app.post("/api/auth/register", (req, res) => {
  try {
    ...
    const newUser: UserProfile = {
      ...
      login: cleanLogin,
      parol: hashPassword(parol.trim()),
      ism: ism.trim(),
      rol: rol as any,
      yaratilganSana: new Date().toISOString().split('T')[0]
    };

    if (rol === "foydalanuvchi") {
...
app.post("/api/auth/login", (req, res) => {
  try {
    ...
    if (!user || !verifyPassword(parol.trim(), user.parol)) {
...
app.post("/api/auth/logout", requireAuth, (req: AuthedRequest, res) => {
  ...
  if (header?.startsWith("Bearer ")) {
    destroySession(header.slice(7).trim());
  }
  return res.json({ success: true });
});

// 2c. Joriy foydalanuvchi
...
app.post("/api/patients/:userId/sync", requireAuth, (req: AuthedRequest, res) => {
  try {
    const { userId } = req.params;
    const { soglik_skrining_tarixi, soglik_kundaligi } = req.body;
    const actor = req.user!;

    if (actor.rol === "foydalanuvchi" && actor.id !== userId) {
      return res.status(403).json({ error: "Faqat o'z profilingizni sinxronlashtira olasiz." });
    }
    if (actor.rol !== "foydalanuvchi" && actor.rol !== "shifokor" && actor.rol !== "admin") {
    ...
    if (users[userIdx].rol !== "foydalanuvchi") {
      return res.status(400).json({ error: "Faqat bemor profillari sinxronizatsiya qilinadi." });
    }

    if (Array.isArray(soglik_skrining_tarixi)) {
      users[userIdx].soglik_skrining_tarixi = normalizeScreeningHistory(soglik_skrining_tarixi);
    }
...
app.get("/api/patients", requireAuth, requireRoles("shifokor", "admin"), (req, res) => {
  try {
    const users = getUsers();
...
app.post("/api/patients/:userId/advice", requireAuth, requireRoles("shifokor"), (req: AuthedRequest, res) => {
  try {
    const { userId } = req.params;
...
app.get("/api/patients/:userId/advices", requireAuth, (req: AuthedRequest, res) => {
  try {
    const { userId } = req.params;
    const actor = req.user!;
    if (actor.rol === "foydalanuvchi" && actor.id !== userId) {
      return res.status(403).json({ error: "Faqat o'z maslahatlaringizni ko'rishingiz mumkin." });
    }
    const advices = getAdvices();
...
app.get("/api/admin/users", requireAuth, requireRoles("admin"), (req, res) => {
  try {
    const users = getUsers();
...
app.put("/api/admin/users/:userId", requireAuth, requireRoles("admin"), (req, res) => {
  try {
    const { userId } = req.params;
...
// 9. Delete account
app.delete("/api/admin/users/:userId", requireAuth, requireRoles("admin"), (req, res) => {
  try {
    const { userId } = req.params;
    const users = getUsers();
 ...
 */
app.post("/api/analyze-complaint", requireAuth, async (req, res) => {
